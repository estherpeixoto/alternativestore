<x-app-layout>
    ...
    <form method="POST" enctype="multipart/form-data" x-data="{ geral: true, imagens: false, tamanhos: false }" action="/dashboard/produtos{{ isset($product->id) ? "/$product->id" : '' }}">
        ...
        <div class="flex items-center justify-center mb-5">
            <button type="button" @click="geral = true; imagens = false; tamanhos = false;"
                :class="{'border-b border-gray-700 text-gray-800': geral, 'hover:text-gray-800 text-gray-400 transition': !geral}"
                class="py-3 text-sm font-semibold uppercase rounded-none focus:outline-none">
                Geral
            </button>

            <button type="button" @click="geral = false; imagens = true; tamanhos = false;"
                :class="{'border-b border-gray-700 text-gray-800': imagens, 'hover:text-gray-800 text-gray-400 transition': !imagens}"
                class="py-3 ml-8 text-sm font-semibold uppercase rounded-none focus:outline-none">
                Imagens
            </button>

            <button type="button" @click="geral = false; imagens = false; tamanhos = true;"
                :class="{'border-b border-gray-700 text-gray-800': tamanhos, 'hover:text-gray-800 text-gray-400 transition': !tamanhos}"
                class="py-3 ml-8 text-sm font-semibold uppercase rounded-none focus:outline-none">
                Tamanhos
            </button>
        </div>
        ...
        <section id="geral" :class="{'block': geral, 'hidden': ! geral}">
            ...
            <div class="mt-5">
                ...
                <x-input id="price" type="text" name="price" value="{{ old('price') ?? (number_format(($product->price ?? 0), 2, '.', ',') ?? '') }}" required />
            </div>
